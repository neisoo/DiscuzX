...
<div class="n5qj_wxgdan cl">
	<a onClick="lbztsx()" class="wxmsx"></a>
...
<!--{if !$subforumonly}-->
	<!--{if empty($_G['forum']['sortmode'])}-->
		<!--{if empty($_G['forum']['picstyle']) || $_G['cookie']['forumdefstyle']}-->
			...
			<!--{if $_G['forum_threadcount']}-->
				...
				<div class="n5sq_ztlb cl">
					...
					<div class="n5sq_htmk cl">
						...
						<!--{eval $isvideo = JudgeVideo($thread['tid']);}-->
						<!--{if $isvideo}-->
						<!--{eval $messag = preg_replace('/\[zhikai_youku(.*?)\]\s*([^\[\<\r\n]+?)\s*((\[\/zhikai_youku\])|\.\.\.)/is','', $post['message']);}-->
							<div class="n5_htspbt cl">
								<p class="htspbt_bt cl"><a href="forum.php?mod=viewthread&tid=$thread[tid]&fromguid=hot&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra">{$thread[subject]}</a></p>
								<p class="htspbt_js cl"><a href="forum.php?mod=viewthread&tid=$thread[tid]&fromguid=hot&{if $_GET['archiveid']}archiveid={$_GET['archiveid']}&{/if}extra=$extra">$messag</a></p>
							</div>
							<div class="n5_htnrsp cl"><!--{hook/forumdisplay_threadnlbsp_mobile $key}--></div>
						<!--{else}-->
			...
			<!--{/if}-->
			<div class="n5sq_plfb cl"><a onClick="nrywfx()"></a></div>
			<script type="text/javascript">
				var jq = jQuery.noConflict(); 
				function nrywfx(){
					jq(".n5qj_lbhflb").addClass("am-modal-active");	
					if(jq(".sharebg").length>0){
						jq(".sharebg").addClass("sharebg-active");
					}else{
						jq("body").append('<div class="sharebg"></div>');
					...
					jq(".sharebg-active,.nrfx_qxan").click(function(){
						jq(".n5qj_lbhflb").removeClass("am-modal-active");	
						setTimeout(function(){
							jq(".sharebg-active").removeClass("sharebg-active");	
							jq(".sharebg").remove();	
						},300);
			...
			<div class="n5qj_lbhflb cl">
				<div class="n5qj_lbhfxz cl">
					<ul>
					<!--{if !$_G['forum']['allowspecialonly']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]" {if $_G[uid]}{else}class="n5app_wdlts"{/if}><span style="background:#3EBBFD;"><i class="iconfont icon-huati"></i></span>{$n5app['lang']['fatiezi']}</a></li><!--{/if}-->
					<!--{if $_G['forum']['threadsorts'] && !$_G['forum']['allowspecialonly']}-->
						<!--{loop $_G['forum']['threadsorts']['types'] $id $threadsorts}-->
							<!--{if $_G['forum']['threadsorts']['show'][$id]}-->
								<li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&extra=$extra&sortid=$id" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>$threadsorts</a></li>
							<!--{/if}-->
						<!--{/loop}-->
					<!--{/if}-->
					<!--{if $_G['group']['allowpostpoll']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=1" {if $_G[uid]}{else}class="n5app_wdlts"{/if}><span style="background:#FF9900;"><i class="iconfont icon-toupiao"></i></span>{lang post_newthreadpoll}</a></li><!--{/if}-->
					<!--{if $_G['group']['allowpostreward']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=3" {if $_G[uid]}{else}class="n5app_wdlts"{/if}><span style="background:#9dca06;"><i class="iconfont icon-xuanshang"></i></span>{lang post_newthreadreward}</a></li><!--{/if}-->
					<!--{if $_G['group']['allowpostdebate']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=5" {if $_G[uid]}{else}class="n5app_wdlts"{/if}><span style="background:#ff6060;"><i class="iconfont icon-taolunqu"></i></span>{lang post_newthreaddebate}</a></li><!--{/if}-->
					<!--{if $_G['group']['allowpostactivity']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=4" {if $_G[uid]}{else}class="n5app_wdlts"{/if}><span style="background:#da99db;"><i class="iconfont icon-huodong"></i></span>{$n5app['lang']['tzlbfhd']}</a></li><!--{/if}-->
					<!--{if $_G['group']['allowposttrade']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=2" {if $_G[uid]}{else}class="n5app_wdlts"{/if}><span style="background:#7b8ce2;"><i class="iconfont icon-shangpin"></i></span>{$n5app['lang']['tzlbfsp']}</a></li><!--{/if}-->
					</ul>
				</div>
		...
		<!--{else}-->
			...
			<div class="n5sq_plfb n5sq_pbfb cl">
				<a onClick="nrywfx()" {if $_G[uid]}{else}class="n5app_wdlts"{/if}></a>
			</div>
			<script type="text/javascript">
				var jq = jQuery.noConflict(); 
				function nrywfx(){
	...
	<!--{eval $nofooter = true;}-->
		...
		<!--{/if}-->
		<div class="n5sq_plfb cl"><a onClick="nrywfx()" {if $_G[uid]}{else}class="n5app_wdlts"{/if}></a></div>
		<script type="text/javascript">
			var jq = jQuery.noConflict(); 
			function nrywfx(){
				jq(".n5qj_lbhflb").addClass("am-modal-active");	
		...
		<div class="n5qj_lbhflb cl">
			<div class="n5qj_lbhfxz n5qj_lbhfxzs cl">
				<ul>
				<!--{if !$_G['forum']['allowspecialonly']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>{$n5app['lang']['fatiezi']}</a></li><!--{/if}-->
				<!--{if $_G['forum']['threadsorts'] && !$_G['forum']['allowspecialonly']}-->
					<!--{loop $_G['forum']['threadsorts']['types'] $id $threadsorts}-->
						<!--{if $_G['forum']['threadsorts']['show'][$id]}-->
							<li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&extra=$extra&sortid=$id" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>$threadsorts</a></li>
					...
					<!--{/loop}-->
				<!--{/if}-->
				<!--{if $_G['group']['allowpostpoll']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=1" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>{lang post_newthreadpoll}</a></li><!--{/if}-->
				<!--{if $_G['group']['allowpostreward']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=3" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>{lang post_newthreadreward}</a></li><!--{/if}-->
				<!--{if $_G['group']['allowpostdebate']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=5" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>{lang post_newthreaddebate}</a></li><!--{/if}-->
				<!--{if $_G['group']['allowpostactivity']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=4" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>{$n5app['lang']['tzlbfhd']}</a></li><!--{/if}-->
				<!--{if $_G['group']['allowposttrade']}--><li><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=2" {if $_G[uid]}{else}class="n5app_wdlts"{/if}>{$n5app['lang']['tzlbfsp']}</a></li><!--{/if}-->
				</ul>
			</div>
			<button class="nrfx_qxan">{$n5app['lang']['sqbzssmqx']}</button>
		</div>
