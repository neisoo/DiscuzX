...
<div class="n5sq_dslb cl">
	...
	<!--{loop $recommend_users $rdu}-->
		<li>
			<div class="dslb_txys cl"><a href="home.php?mod=space&uid={$rdu['uid']}&do=profile"><img src="uc_server/avatar.php?uid={$rdu['uid']}&size=middle"/></a></div>
			<div class="dslb_hyxx cl">
				<span class="y">{echo date("Y-m-d H:i",$rdu['dateline'])}</span>
				<div class="dslb_hypf cl"><a href="home.php?mod=space&uid={$rdu['uid']}" class="rate_user">{$rdu['username']}</a></div>
				<p>$log[reason]</p>
			</div>
		</li>
	<!--{/loop}-->
