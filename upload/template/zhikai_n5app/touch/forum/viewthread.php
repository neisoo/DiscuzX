...
<style type="text/css">
.bg {padding-top: 0;}
.n5sq_xlcd {top: 10px;z-index: 2;position: fixed;}
</style>
<div class="n5qj_wxgdan cl">
	<a id="a" href="javascript:void(0)" class="wxmsx kjgdcz n5sq_xlcf" onclick="n5sq_xlkzList(this)"></a>
...
<div class="n5sq_xlcd" id="dropdown-a">
		<ul>
			...
			<li><a <!--{if $isapp}-->onclick="ShareBnt()"<!--{else}-->onClick="nrywfx()"<!--{/if}-->><i class="iconfont icon-n5appxlfx"></i>{$n5app['lang']['nrxlfxzt']}</a></li>
...
<div class="n5sq_ztnr cl">
                        ...
                        <!--{if $post['first']}-->
							...
							<!--{if $recommend_users}-->
								<div class="n5sq_ztdz cl">
									<h2><span class="y">{$n5app['lang']['ztnrdztzid']} $_G[tid]</span><a href="forum.php?mod=misc&action=recommend&do=add&tid=$_G[tid]&hash={FORMHASH}" class="{if $_G['uid']}dialog{else}n5app_wdlts{/if}" {if C::t('forum_memberrecommend')->fetch_by_recommenduid_tid($_G['uid'], $_G['tid'])}style="color: #a9a9a9;background: #f1f1f1;"{/if}><i class="iconfont icon-n5appnrsc"></i>{if C::t('forum_memberrecommend')->fetch_by_recommenduid_tid($_G['uid'], $_G['tid'])}{$n5app['lang']['ztnrdzzgty']}{else}{$n5app['lang']['ztnrdzzgtz']}{/if}</a></h2>
									<ul>
	...
	<!--{if $post['first']}-->
		...
		{if $n5app['sfkqapptg']}
		<div class="n5sq_apptg cl">
			<div class="n5sq_apptgtp"><img src="{$n5app['apptgtbimg']}"></div>
			<div class="n5sq_apptgjs">
				<p>{$n5app['lang']['apptgdkapp']}<!--{if $_G[forum_thread][allreplies] == 0 }-->{$n5app['lang']['apptgcydsh']}<!--{else}-->{$n5app['lang']['apptgck']}<i>$_G[forum_thread][allreplies]</i>{$n5app['lang']['apptgckhfw']}<!--{/if}--></p>
			</div>
			<div class="n5sq_apptgxz cl">
				<a href="{$n5app['apptgxzdz']}" class="nbg">{$n5app['lang']['apptgljxz']}</a>
				<a onclick="clickautohide(5)" class="appbg">{$n5app['lang']['apptgdkap']}</a>
			</div>
		</div>
		...
		<script>
			...
			function clickautohide(i){
				...
				switch(i){
					case 5:
					tip = "{$n5app['lang']['apptgyxzts']}";
					break;
				}
				ZENG.msgbox.show(tip, i, 3500);
			}
		</script>
		{/if}
...
<!--{if $isapp}-->
<style type="text/css">
.n5sq_apptg {display:none;}
</style>
<!--{else}-->	
<style type="text/css">
.n5sq_nrbk,.n5sq_hfsx,.n5sq_ztnr .n5sq_hfys,.page,.n5sq_lzds,.n5sq_dsjl,.n5sq_sfts,.n5sq_ztdz {display:none;}
</style>
<!--{/if}-->
{/if}
...
<div class="n5sq_nrdb cl">
	<a href="javascript:history.back();" class="nrdb_fh"><i class="iconfont icon-n5appnrfho"></i><p>{$n5app['lang']['qjfanhui']}</p></a>
	...
	<a <!--{if $isapp}-->onclick="ShareBnt()"<!--{else}-->onClick="nrywfx()"<!--{/if}-->><i class="iconfont icon-n5appnrfxo"></i><p>{$n5app['lang']['sqnrdbfxnr']}</p></a>
</div>
