...
<div class="n5gr_kjcz cl">
	<div class="kjcz_czxm cl">
		<ul>
			...
			<li><a href="home.php?mod=spacecp&amp;ac=poke&amp;op=send&amp;uid=$space[uid]&amp;handlekey=propokehk_$space[uid]"><img nodata-echo="yes" src="template/zhikai_n5app/images/kjcz_dgzh.png"><p>{$n5app['lang']['dagezhaohu']}</p></a></li>
		</ul>
	</div>
...
<div class="n5gr_kjsj cl">
	...
	<div class="kjsj_hyzl cl">
		<div class="hyzl_btbj cl">
			<span class="z">{$n5app['lang']['grzxxzbt']}</span>
		</div>
		<div class="hyzl_xzlb cl">
			<!--{loop $space['medals'] $medal}-->
				<a href="home.php?mod=medal"><img src="{STATICURL}/image/common/$medal[image]"><p>$medal[name]</p></a>
			<!--{/loop}-->
		</div>
	</div>
	<!--{/if}-->
...
<div class="n5qj_wbys cl">
	...
	<!--{/if}-->
	<!--{if $n5app['dbdhssl'] == 1}--><a href="<!--{if $_G[uid]}-->home.php?mod=space&uid=$_G[uid]&do=profile&mycenter=1<!--{else}-->member.php?mod=logging&action=login<!--{/if}-->"  class="<!--{if $_G[uid]}-->qjyw_txys <!--{if $space[self]}-->on<!--{/if}--><!--{else}-->qjyw_wdkz<!--{/if}-->"><!--{if $_G[uid]}--><!--{avatar($_G[uid])}--><!--{else}--><i class="iconfont icon-n5appwd"></i><!--{/if}--><br/>{$n5app['lang']['qjwode']}<!--{if $_G[member][newprompt]}--><b></b><!--{/if}--><!--{if $_G[member][newpm]}--><b></b><!--{/if}--></a>
...
<div class="n5gr_wddh cl">
	...
	<a href="home.php?mod=medal&action=log&mobile=2" class="n5ico">
		<div class="wddh_dhtb"><i style="color: #be64ff" class="iconfont icon-n5appwdxz"></i></div>
		<div class="wddh_dhbt">{lang my_medals}</div>
		<div class="wddh_ljjt">
			<i class="iconfont icon-n5appxy"></i>
		</div>		
	</a>
	<a href="plugin.php?id=zhikai_sms:verify&motion=bin" class="n5ico">
		<div class="wddh_dhtb"><i style="color:#b8da42" class="iconfont icon-shouji"></i></div>
		<div class="wddh_dhbt">{$n5app['lang']['grzxsjbd']}</div>
		<div class="wddh_ljjt">
			<!--{if $_G['uid'] && in_array('zhikai_sms',$_G['setting']['plugins']['available'])}-->
			<!--{eval $mobile_state = DB::result_first('select mobile_state from %t where uid=%d', array('zhikai_userdata', $_G['uid']));}-->
				<!--{if $mobile_state == 0}--><span>{$n5app['lang']['grzxsjbdts']}</span><!--{/if}-->
			<!--{/if}-->
			<i class="iconfont icon-n5appxy"></i>
		</div>		
	</a>
...
<div class="n5qj_wbys cl">
	...
	<!--{elseif $n5app['dbdhdsl'] == 3}--><a href="{$n5app['dbdhsasllj']}" class="qjyw_fxxx"><i class="iconfont icon-n5appdsl"></i><br/>{$n5app['dbdhsaslwz']}</a>
	<!--{/if}-->
	<!--{if $n5app['dbdhssl'] == 1}--><a href="<!--{if $_G[uid]}-->home.php?mod=space&uid=$_G[uid]&do=profile&mycenter=1<!--{else}-->member.php?mod=logging&action=login<!--{/if}-->" class="<!--{if $_G[uid]}-->qjyw_txys on<!--{else}-->qjyw_wdkz<!--{/if}-->"><!--{if $_G[uid]}--><!--{avatar($_G[uid])}--><!--{else}--><i class="iconfont icon-n5appwd"></i><!--{/if}--><br/>{$n5app['lang']['qjwode']}</a>
	<!--{elseif $n5app['dbdhssl'] == 2}--><a href="{$n5app['dbdhssllj']}" class="qjyw_fxxx"><i class="iconfont icon-n5appfx"></i><br/>{$n5app['dbdhsslwz']}</a><!--{/if}-->
</div>
<div class="wbys_yqmb"></div>
<!--{/if}-->
