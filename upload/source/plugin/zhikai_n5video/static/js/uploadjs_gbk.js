...
SWFUpload.Console.writeLine = function (message) {
	...
	try {
		...
		upmsg("Exception: " + ex.name + " Message: " + ex.message);
	}
};
...
(function($){
	...
	var loadUploadDom = function (){
		if(!USE_STREAM_UPLOAD){
			...
			var htmlStr = '+ 选择文件<input class="input-file" id="fileInput" type="file" name="FileData" style="opacity:0;display:block;position:relative;top:-28px;width:100%;height:26px;">';
			$("#uploadControl").html(htmlStr);
		}
	...
	var fileSizeOver = function(file){
        if(file){
			...
			if(fileSize >fileSizeLimit){
				upmsg("视频文件大小超过限制，请缩小视频文件后重新选择文件");
				result = true;
			}
			return result;
		}
		...
		if(!isCancelUpload){
			...
			$.ajax({
				...
				success:function(data) { 
					...
					if (data.video_id) {
						//var tpl = '<div class="upmsg upmsg-success"><h1>上传成功！</h1><br>';
						...
						upmsg("上传任务失败，更换其它视频在试试");
						setTimeout(function() {
							window.location.reload();
						},1500);
						//$("#upload-status-wraper").html('<div class="alert alert-success"><h1></h1></div>');
					}
				}
			});
		}
    ...
    var createUploadTask = function (){
        ...
        var params = {
                ...
                tags: $("#input02").val(),
                category: $("#category-node").val(),
                copyright_type: $("copyright_type2").val(),
                public_type: $("input[name='public_type']:checked").val(),
                client_id: uploadOptions["client_id"],
                access_token: uploadOptions["access_token"],
                file_name: $("#fileInput").val(),
                isweb:1,
        };
		...
		if(params["public_type"] == "password") {
			params["watch_password"] = $("input[name='watch_password']").val();
		}
		obj = null;
		if(USE_STREAM_UPLOAD){
			var file = $("#fileInput")[0].files[0];
...
//						}
    ...
    var initUploadInfo = function (){
		...
		tpl +=  '<span id="tdCurrentSpeed">00.00 kbit/s</span> | <span id="tdTimeRemaining">00:00:00</span> | <span id="tdPercentUploaded">00.00 %</span> | <span id="tdSizeUploaded">00.00 KB</span> / <span id="bytestotal">00.00 KB</span></div><button type="button" style="float:right;" class="btn btn-primary start" id="btn-upload-stop"><span>取消上传</span></button>';
	...
	function uploadSuccess(file, serverData){
		var data = eval("(" + serverData + ")");
		 if (data.upload_server_name) {
			...
			upmsg("文件上传失败error="+data.error.code);
		 }
	}
