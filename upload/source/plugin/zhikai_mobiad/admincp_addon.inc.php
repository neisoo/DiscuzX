 ...
 *
 *      $Id: admincp_addon.php 34498 2014-05-12 02:51:02Z nemohou $
 */
...
$_config['addon'] = array(
    'xx1' => array(
	'website_url' => 'addon.php',
	'download_url' => 'addon.php',
	'download_ip' => '',
    )
);
...
function pcloudaddons_getuniqueid() {
	global $_G;
	if(PCLOUDADDONS_WEBSITE_URL == 'addon.php') {
...
if(!$poperation) {
	...
	if($step == 0) {
		cpmsg('cloudaddons_downloading', "action=plugins&operation=config&do={$_GET['do']}&identifier={$_GET['identifier']}&poperation=download&pmod=admincp_addon&addonids=$_GET[addonids]&i=$addoni&step=1&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'], 'loading', array('addonid' => $_GET['key'].'.'.$_GET['type']), '<div>0%</div>', FALSE);
	} elseif($step == 1) {
		...
		if(!$end) {
			$packnum++;
			cpmsg('cloudaddons_downloading', "action=plugins&operation=config&do={$_GET['pdo']}&identifier={$_GET['identifier']}&poperation=download&pmod=admincp_addon&addonids=$_GET[addonids]&i=$addoni&step=1&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'].'&num='.$packnum, 'loading', array('addonid' => $_GET['key'].'.'.$_GET['type']), '<div>'.$percent.'%</div>', FALSE);
		} else {
			cpmsg('cloudaddons_installing', "action=plugins&operation=config&do={$_GET['pdo']}&identifier={$_GET['identifier']}&poperation=download&pmod=admincp_addon&addonids=$_GET[addonids]&i=$addoni&end=$end&step=2&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'], 'loading', array('addonid' => $_GET['key'].'.'.$_GET['type']), FALSE);
		}
	} elseif($step == 2) {
		$tmpdir = DISCUZ_ROOT.'./data/download/'.$_GET['rid'];
		...
		if($unwriteabledirs) {
			if(!submitcheck('settingsubmit')) {
				...
				siteftp_form("action=plugins&operation=config&do={$_GET['pdo']}&identifier={$_GET['identifier']}&poperation=download&pmod=admincp_addon&addonids=$_GET[addonids]&i=$addoni&end=".rawurlencode($_GET['end'])."&step=2&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp']);
				exit;
			} else {
		...
		if(count($addonids) - 1 > $addoni) {
			$addoni++;
			cpmsg('cloudaddons_downloading', "action=plugins&operation=config&do={$_GET['pdo']}&identifier={$_GET['identifier']}&poperation=download&pmod=admincp_addon&&addonids=$_GET[addonids]&i=$addoni&step=1&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'], 'loading', array('addonid' => $_GET['key'].'.'.$_GET['type']), FALSE);
		}
