...
function zknvbing5_v2($data){
	global $_G;
	$tids = array();
	foreach($data as $k => $list){
		foreach($list['threadlist'] as $k => $v){
			$tids[] = $v['tid'];
		}
	}
	...
	unset($result);
	foreach($data as $k => $list){
		foreach($list['threadlist'] as $key => $thread){
			$_G['setting']['pluginhooks']['guide_threadn5lbsp_mobile'][$key] = forumdisplay_replace(
